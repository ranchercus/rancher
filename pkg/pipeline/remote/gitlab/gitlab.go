...
func (c *client) GetPipelineFileInRepo(repoURL string, ref string, accessToken string, subPath ...string) ([]byte, error) {
	...
	file, err := c.getFileFromRepo(path + utils.PipelineFileYml, project, ref, accessToken)
	if err != nil {
		//look for both suffix
		file, err = c.getFileFromRepo(path + utils.PipelineFileYaml, project, ref, accessToken)
	}
	if err != nil {
...
func (c *client) SetPipelineFileInRepo(repoURL string, branch string, accessToken string, content []byte, subPath ...string) error {
	...
	currentFile, err := c.getFileFromRepo(path + utils.PipelineFileYml, project, branch, accessToken)
	...
	if err != nil {
		...
		//look for both suffix
		currentFile, err = c.getFileFromRepo(path + utils.PipelineFileYaml, project, branch, accessToken)
		if err != nil {
			if httpErr, ok := err.(*httperror.APIError); !ok || httpErr.Code.Status != http.StatusNotFound {
				return err
			}
